...
resource "azurerm_subnet" "aca_pls" {
  // workaround: operate subnets one after another
  // https://github.com/hashicorp/terraform-provider-azurerm/issues/3780
  depends_on = [
...
data "azurerm_lb" "aca" {
  ...
  resource_group_name = "MC_${split(".", jsondecode(azapi_resource.aca_env.output).properties.defaultDomain)[0]}-rg_${split(".", jsondecode(azapi_resource.aca_env.output).properties.defaultDomain)[0]}_${split(".", jsondecode(azapi_resource.aca_env.output).properties.defaultDomain)[1]}"
}

resource "azurerm_private_link_service" "aca" {
  name                = "pls-aca"
  resource_group_name = azurerm_resource_group.aca_poc.name
  location            = azurerm_resource_group.aca_poc.location
  ...
  load_balancer_frontend_ip_configuration_ids = [data.azurerm_lb.aca.frontend_ip_configuration.0.id]

  nat_ip_configuration {
    name                       = "primary"
    private_ip_address_version = "IPv4"
    subnet_id                  = azurerm_subnet.aca_pls.id
    primary                    = true
  }
}

# The followings are other VNet resources

resource "azurerm_resource_group" "aca_poc_other" {
  name     = "${local.rg.name}-other"
  location = local.rg.location
}

module "other_vnet_subnet_addrs" {
  source = "hashicorp/subnets/cidr"
...
resource "azurerm_virtual_network" "other" {
  ...
  location            = azurerm_resource_group.aca_poc_other.location
  address_space       = [module.other_vnet_subnet_addrs.base_cidr_block]
}
...
resource "azurerm_private_endpoint" "aca" {
  ...
  location            = azurerm_resource_group.aca_poc_other.location
  resource_group_name = azurerm_resource_group.aca_poc_other.name
  subnet_id           = azurerm_subnet.other_default.id

  private_service_connection {
    name                           = "pe-connection-aca"
    private_connection_resource_id = azurerm_private_link_service.aca.id
    is_manual_connection           = false
  }
}

resource "azurerm_private_dns_zone" "aca_env_other" {
  name                = jsondecode(azapi_resource.aca_env.output).properties.defaultDomain
  resource_group_name = azurerm_resource_group.aca_poc_other.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "aca_env_other" {
  name                  = "pdnsz-link-aca-env-other"
  resource_group_name   = azurerm_resource_group.aca_poc_other.name
  private_dns_zone_name = azurerm_private_dns_zone.aca_env_other.name
  virtual_network_id    = azurerm_virtual_network.other.id
}

resource "azurerm_private_dns_a_record" "aca_env_other" {
  name                = "*"
  zone_name           = azurerm_private_dns_zone.aca_env_other.name
  resource_group_name = azurerm_resource_group.aca_poc_other.name
  ttl                 = 300
  records             = [azurerm_private_endpoint.aca.private_service_connection.0.private_ip_address]
}

resource "azurerm_network_security_group" "default_other" {
  name                = "nsg-default"
  resource_group_name = azurerm_resource_group.aca_poc_other.name
  location            = azurerm_resource_group.aca_poc_other.location
...
resource "azurerm_public_ip" "client_other_vnet" {
  name                = "pip-client-other-vnet"
  resource_group_name = azurerm_resource_group.aca_poc_other.name
  location            = azurerm_resource_group.aca_poc_other.location
  allocation_method   = "Dynamic"
}

resource "azurerm_network_interface" "client_other_vnet" {
  name                          = "nic-client-other-vnet"
  resource_group_name           = azurerm_resource_group.aca_poc_other.name
  location                      = azurerm_resource_group.aca_poc_other.location
  enable_accelerated_networking = true
  ...
  ip_configuration {
    ...
    subnet_id                     = azurerm_subnet.other_default.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.client_other_vnet.id
  }
}

resource "azurerm_network_interface_security_group_association" "client_other_vnet" {
  network_interface_id      = azurerm_network_interface.client_other_vnet.id
  network_security_group_id = azurerm_network_security_group.default_other.id
}

resource "azurerm_linux_virtual_machine" "client_other_vnet" {
  name                            = "vm-client-other-vnet"
  resource_group_name             = azurerm_resource_group.aca_poc_other.name
  location                        = azurerm_resource_group.aca_poc_other.location
  size                            = "Standard_D2ds_v4"
  admin_username                  = var.admin_username
  ...
  network_interface_ids = [
    azurerm_network_interface.client_other_vnet.id,
  ]
...
resource "azurerm_virtual_machine_extension" "aad_ssh_login_client_other_vnet" {
  name                       = "AADSSHLoginForLinux"
  virtual_machine_id         = azurerm_linux_virtual_machine.client_other_vnet.id
  publisher                  = "Microsoft.Azure.ActiveDirectory"
  type                       = "AADSSHLoginForLinux"
  type_handler_version       = "1.0"
  auto_upgrade_minor_version = true
